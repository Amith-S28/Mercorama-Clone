// app/dashboard/_tools/fund-my-export-tool.tsx
'use client';
    ...
    <div className="rounded-xl border bg-card p-4 sm:p-5 space-y-3">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{match.program.provider}</p>
        </div>
        <span className={cn('shrink-0 rounded-full px-2.5 py-0.5 text-xs font-semibold', fit.className)}>
          {fit.label}
        </span>
      </div>
      <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{match.ai_snippet}</p>
      ...
      <div className="flex items-center justify-between pt-2 border-t gap-3">
        <span className="text-xs text-muted-foreground capitalize">{match.program.program_type}</span>
...
function ResultSkeleton() {
  return (
    <div className="space-y-3">
      ...
      <p className="text-center text-xs text-muted-foreground pt-2">Matching you to available programs...</p>
    </div>
  );
}

// ─── Upgrade gate (shown to Starter users) ────────────────────────────────────
...
function UpgradeGate() {
  ...
  return (
    <div className="rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-gradient-to-br from-teal-50 to-emerald-50 dark:from-teal-950/30 dark:to-emerald-950/30 p-6 max-w-xl mx-auto mt-8">
      <div className="flex items-center gap-3 mb-2">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-100 dark:bg-emerald-900/40 shrink-0">
          <DollarSign className="h-5 w-5 text-emerald-700 dark:text-emerald-400" />
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-bold">Fund My Export</span>
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-5 leading-relaxed">
        See every grant and program you qualify for — up to $165,000 may be available for your next market entry.
      </p>
      ...
      <Link href="/beta" onClick={() => trackEvent('fund_my_export_upgrade_clicked', { source: 'standalone_page' })}>
...
export function FundMyExportTool() {
  const [userId, setUserId]   = useState<string | null>(null);
  const [isGrowth, setIsGrowth] = useState<boolean | null>(null); // null = loading
  ...
  useEffect(() => {
    fetch('/api/me')
      ...
      .then((me) => {
        const isTeamOrEnterprise = me?.plan === 'team' || me?.plan === 'enterprise';
        if (isTeamOrEnterprise) {
          setIsGrowth(true);
          trackEvent('fund_my_export_viewed', { plan: 'growth', surface: 'standalone' });
        } else {
          setIsGrowth(false);
          trackEvent('fund_my_export_viewed', { plan: 'starter', surface: 'standalone' });
          return;
        }

        // Load Supabase user id + run count
        supabase.auth.getUser().then(({ data }) => {
          if (!data.user) return;
          setUserId(data.user.id);
          getRunStatus(data.user.id)
  ...
  // ── Upgrade gate for Starter users ───────────────────────────────────────
  if (!isGrowth) {
    return (
      <div>
        <div className="mb-8">
          <div className="mb-4 inline-flex items-center gap-2 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 px-3 py-1.5">
          ...
          <p className="max-w-2xl text-lg leading-relaxed text-muted-foreground">
            Match to every Canadian government grant, guarantee, and support program you qualify for.
          </p>
        </div>
        <UpgradeGate />
      </div>
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <div className="mt-4 inline-flex items-center gap-2 rounded-full border bg-muted/50 px-4 py-1.5 text-sm">
          ...
          <span><span className="font-semibold">{runsUsed} / 20</span> searches used this month</span>
            ...
            <CardContent className="pt-6">
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={formValues.province} onValueChange={(val) => setFormValues((v) => ({ ...v, province: val }))}>
                    <SelectTrigger><SelectValue placeholder="Select province or territory" /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={formValues.sector} onValueChange={(val) => setFormValues((v) => ({ ...v, sector: val }))}>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select onValueChange={(val) => setFormValues((v) => ({ ...v, revenue: Number(val) }))}>
                    <SelectTrigger><SelectValue placeholder="Select revenue range" /></SelectTrigger>
                    <SelectContent>
                      {REVENUE_OPTIONS.map((o) => <SelectItem key={o.label} value={String(o.value)}>{o.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormValues((v) => ({ ...v, employees: e.target.value ? Number(e.target.value) : null }))}
                  />
                </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer rounded-lg border p-3">
                  <input type="checkbox" checked={formValues.isTendering}
        ...
        <div className="lg:col-span-3">
          {submitState === 'idle' && (
            <div className="flex min-h-[400px] flex-col items-center justify-center gap-4 rounded-2xl border border-dashed p-12 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Fill in your details to see matched funding programs →</p>
              </div>
            </div>
          )}
          ...
          {submitState === 'limit' && (
            <div className="rounded-xl border border-amber-300 bg-amber-50 dark:border-amber-700 dark:bg-amber-900/20 p-6 space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-amber-800 dark:text-amber-300">You've used all 20 Fund My Export searches this month.</p>
                  <p className="text-sm text-amber-700 dark:text-amber-400 mt-1">Resets on {resetsOn}</p>
                </div>
              </div>
              <p className="text-sm text-amber-800 dark:text-amber-300">
                The Trade Commissioner Service offers free advisory to all Canadian exporters:
              </p>
              <a href="https://www.tradecommissioner.gc.ca" target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-sm font-medium text-amber-800 dark:text-amber-300 hover:underline">
                Contact TCS <ExternalLink className="h-3.5 w-3.5" />
              </a>
            </div>
          )}
          ...
          {submitState === 'empty' && (
            <div className="rounded-xl border border-dashed p-8 text-center space-y-3">
              <p className="text-muted-foreground">No exact program matches for your profile right now.</p>
              <a href="https://www.tradecommissioner.gc.ca" target="_blank" rel="noopener noreferrer"
                ...
                <div>
                  ...
                  {fromCache && (
                    <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5 flex items-center gap-1">
                      <RefreshCw className="h-3 w-3" /> Results from cache
                    </p>
                  )}
                </div>
      ...
      <ToolCrossPromo currentTool="fund-my-export" />
    </div>
  );
}
