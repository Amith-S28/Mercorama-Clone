...
import { Loader2, MapPin, CheckCircle2, ArrowRight, TrendingUp, ShieldCheck, Truck, Users } from 'lucide-react';
...
export default function CanadaExplorerPage() {
  ...
  // Auto-trigger API (with 24h cache)
  useEffect(() => {
    if (!snapshot) return;
    setChecked(true);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
      <WorkflowBanner
        ...
        ctaLabel={selected ? 'Continue to Global Markets' : undefined}
        ctaHref={selected ? `/dashboard?view=global-markets&product=${encodeURIComponent(snapshot?.productDescription ?? '')}` : undefined}
      />
      <div className="mb-8">
              ...
              >
                <CardContent className="pt-5 space-y-3">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={cn('rounded-full px-2 py-0.5 text-[10px] font-semibold', demand.className)}>
                          {demand.label}
                        </span>
                      </div>
                  ...
                  <div className="rounded-md bg-[#01696f]/5 border border-[#01696f]/20 px-3 py-2">
                    <p className="text-xs font-medium text-[#01696f] dark:text-[#4f98a3]">
                      {market.recommendedEntryStrategy}
                    </p>
                  </div>
                  ...
                  {/* Selected CTA */}
                  {isSelected && (
                    <Link href={`/dashboard?view=global-markets&product=${encodeURIComponent(snapshot?.productDescription ?? '')}`}>
                      <Button size="sm" className="w-full gap-1.5 mt-1">
                        Explore global markets for {market.province} <ArrowRight className="h-3.5 w-3.5" />
                      </Button>
                    </Link>
                  )}
                </CardContent>
