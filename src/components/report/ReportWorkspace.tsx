...
export function ReportWorkspace() {
    ...
    try {
      ...
      if (smeId) {
        const assessRes = await fetch(
          `/api/assessment?smeId=${encodeURIComponent(smeId)}`,
        );
        if (assessRes.ok) {
  ...
  const headerRef = useGsapReveal<HTMLElement>({
    direction: "down",
    distance: 15,
  });
    ...
    <div className="page-container">
      ...
      {activeTab === "history" ? (
        ...
        >
          <div className="bento-grid">
            <div className="bento-card--span-12">
              <CountryTradeHistoryDashboard
                countryIso3={sme?.targetCountry ?? selectedCountry ?? "JPN"}
              />
            </div>
          </div>
        </motion.div>
