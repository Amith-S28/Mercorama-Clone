...
export function TradeIntelDashboard({
  countryIso3,
  hsCode,
}: TradeIntelDashboardProps) {
  ...
  const isBillion = data?.importVolumeUsd
    ? data.importVolumeUsd > 1000000000
    : false;
  const targetValue = data?.importVolumeUsd
    ? isBillion
      ? data.importVolumeUsd / 1000000000
      : data.importVolumeUsd / 1000000
    : 0;

  const { ref: volumeRef, value: volumeValue } =
    useGsapCounter<HTMLHeadingElement>({
      end: targetValue,
    ...
    async function load() {
      ...
      try {
        ...
        setData({
          ...
          dataOrigin:
            summaryRes.headers.get("data-origin") === "cache"
              ? "cache"
              : summary.dataOrigin ||
                summaryRes.headers.get("data-origin") ||
                "unknown",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div className="bento-grid">
        {/* Hero Metric */}
        <div
          className="bento-card bento-card--span-4"
          style={{
            display: "flex",
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            >
              {data.yoyChange >= 0 ? "▲" : "▼"}{" "}
              {Math.abs(data.yoyChange * 100).toFixed(1)}%
            </span>
            <span
              style={{ fontSize: "0.8125rem", color: "var(--text-secondary)" }}
            >
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data.trend}>
                  ...
                  <Line
                    ...
                    strokeWidth={3}
                    dot={{ r: 4, fill: "var(--accent-premium)" }}
                  />
                </LineChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="bento-card bento-card--span-12">
          ...
          >
            <div>
              ...
              >
                Top countries exporting HS {hsCode} products to the targeted
                market of {countryIso3}
              </p>
            </div>
          </div>
          ...
          <div style={{ overflowX: "auto" }}>
            ...
            >
              <thead>
                ...
                >
                  <th style={{ padding: "0.75rem 0.5rem", fontWeight: 500 }}>
                    Rank
                  </th>
                  <th style={{ padding: "0.75rem 0.5rem", fontWeight: 500 }}>
                    Country
              ...
              <tbody ref={tableRef}>
                {data.partners.slice(0, 10).map((p, i) => {
                  const pct =
                    data.importVolumeUsd > 0
                      ? (p.value / data.importVolumeUsd) * 100
                      : 0;
                  return (
                    <tr
                      key={p.country}
                      className="opacity-0"
                      style={{
                        borderBottom: "1px solid var(--border-low-contrast)",
                    ...
                    >
                      ...
                      <td
                        style={{ padding: "0.625rem 0.5rem", fontWeight: 600 }}
                      >
                        {p.country}
                      </td>
                      <td
