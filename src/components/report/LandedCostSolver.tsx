...
export function LandedCostSolver({ sme }: LandedCostSolverProps) {
  ...
  const [volatility90d, setVolatility90d] = useState<number | null>(
    fallback.volatility90d,
  );
    ...
    async function loadMarketInputs() {
      ...
      try {
        ...
        const origins = [freightRes, tariffRes, ratesRes, cbamRes]
          ...
          .filter(Boolean);
        if (
          origins.includes("live") ||
          origins.includes("live-usda-agtransport")
        )
          setDataOrigin("live");
        else if (origins.includes("mock-fallback"))
          setDataOrigin("mock-fallback");
        else if (origins.includes("historical-offline"))
          setDataOrigin("historical-offline");

        if (freightRes.ok) {
          const freightData = (await freightRes.json()) as {
        ...
        if (ratesRes.ok) {
          ...
          if (ratesData.volatility30d != null)
            setVolatility30d(ratesData.volatility30d);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        ...
        <FormattedNumberInput value={quantity} onChange={setQuantity} min={1} />
        <Slider
      ...
      >
        ...
        >
          <Row
            label="Unit freight"
            value={`$${result.unitFreightCost.toFixed(2)}`}
          />
          <Row
            label="Tariff / unit"
          ...
          <Row
            label="Actual margin"
            value={`${result.actualMargin.toFixed(1)}%`}
          />
