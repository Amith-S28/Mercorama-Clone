...
function tierFill(tier: ReturnType<typeof getRiskTierForNumericId>): string {
  ...
  return (
    TIER_COLORS[tier === "restricted" ? "restricted" : tier] ??
    TIER_COLORS["no-data"]
  );
}

function getRoughCentroid(
  feature: Feature,
  ...
  if (geom.type === "Polygon") {
    coords =
      geom.coordinates[0]?.[Math.floor((geom.coordinates[0]?.length || 0) / 2)];
  } else if (geom.type === "MultiPolygon") {
    coords =
      geom.coordinates[0]?.[0]?.[
        Math.floor((geom.coordinates[0]?.[0]?.length || 0) / 2)
      ];
  }
  if (coords) {
...
export function EdcCountryRiskMap({
  ...
  const knownNumericIds = useMemo(
    () => new Set(Object.values(ISO3_TO_NUM)),
    [],
  );
  ...
  const targetCountryFeature = useMemo(() => {
    ...
    return worldData.features.find((f: Feature) => String(f.id) === numId) as
                ...
                >
                  <g transform={zoom.toString()}>
                    <Mercator
                      ...
                      scale={110}
                      translate={[width / 2, mapHeight / 1.55]}
                    >
                      {(mercator) => (
                        <g>
                          {mercator.features.map(
                            ({ feature: country, path }, i) => {
                              const numId = String(
                              ...
                              return (
                                <path
                                  ...
                                  fillOpacity={
                                    isKnown ? (isSelected ? 0.95 : 0.72) : 1.0
                                  }
                                  stroke={
                                    isSelected
                                      ? "var(--accent-premium)"
                                  ...
                                  strokeWidth={
                                    isSelected
                                      ? 1.5 / zoom.transformMatrix.scaleX
                                      : 0.5 / zoom.transformMatrix.scaleX
                                  }
                                  style={{
                          ...
                          {targetCentroid && (
                            <g>
                              {(() => {
                                const from = mercator.projection([
                                  CANADA.lon,
                                  CANADA.lat,
                                ]);
                                const to = mercator.projection([
                                  targetCentroid.lon,
                                  targetCentroid.lat,
                                ]);
                                ...
                                return (
                                  <>
                                    ...
                                    <circle
                                      cx={from[0]}
                                      cy={from[1]}
                                      r={3 / zoom.transformMatrix.scaleX}
                  ...
                  <g transform={`translate(12, ${mapHeight - 12})`}>
                    {(["open", "watch", "restricted"] as const).map(
                      (tier, i) => (
