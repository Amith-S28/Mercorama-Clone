...
export function CountryPlaybook({ iso3 }: CountryPlaybookProps) {
  if (!iso3) {
    return (
      ...
      >
        ...
        >
          Select a country on the map to view FTA status, tariff defaults, and
          market signals.
        </p>
      </div>
    );
  ...
  const tierColor =
    TIER_COLORS[
      tier === "restricted" || tier === "blocked" ? "restricted" : tier
    ] ?? TIER_COLORS["no-data"];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className="mono-label"
          style={{ fontSize: "0.625rem", color: tierColor }}
        >
          {tier}
        </span>
      </div>
      ...
      >
        ...
        <PlaybookRow
          ...
          value={`${(profile.tariffRateDefault * 100).toFixed(1)}%`}
        />
        <PlaybookRow
          label="Freight (FEU)"
          value={`$${profile.freightRateCadPerFeu.toLocaleString()} CAD`}
        />
        <PlaybookRow
          label="Import volume"
          value={`$${(profile.comtradeImportVolumeUsd / 1e6).toFixed(0)}M`}
        />
        <PlaybookRow
          label="YoY change"
        ...
        <PlaybookRow
          label="Sanctions"
          value={profile.sanctionsClear ? "Clear" : "Review required"}
        />
      </dl>
