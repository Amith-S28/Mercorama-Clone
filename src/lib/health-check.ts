...
const PROBES: ServiceProbe[] = [
  ...
  },
  {
    ...
    probe: async () => {
      ...
      try {
        const res = await fetch(
          `https://v6.exchangerate-api.com/v6/${env.EXCHANGE_RATE_API_KEY}/latest/CAD`,
          {
            signal: AbortSignal.timeout(5000),
          },
        );
  ...
  },
  {
    ...
    probe: async () => {
      ...
      try {
        const res = await fetch(
          `https://api.trade.gov/consolidated_screening_list/search?api_key=${env.CSL_API_KEY}&size=1`,
          {
            signal: AbortSignal.timeout(5000),
          },
        );
  ...
  },
  {
    ...
    probe: async () => {
      ...
      try {
        const res = await fetch(
          `https://api.wto.org/timeseries/v1/indicators?subscription-key=${env.WTO_API_KEY}`,
          {
            signal: AbortSignal.timeout(5000),
          },
        );
  ...
  },
  {
    ...
    probe: async () => {
      ...
      try {
        const res = await fetch(
          "https://agtransport.usda.gov/resource/dtp5-fwp8.json?$limit=1",
          {
            signal: AbortSignal.timeout(5000),
          },
        );
  ...
  },
  {
    ...
    probe: async () => {
      ...
      try {
        const res = await fetch(
          "https://hts.usitc.gov/reststop/search?keyword=0101",
          {
            signal: AbortSignal.timeout(5000),
          },
        );
...
export async function checkApiHealth(): Promise<ApiServiceHealth[]> {
  ...
  return Promise.all(
    PROBES.map(async (service): Promise<ApiServiceHealth> => {
      const isKeyConfigured = service.keyEnv
        ? isServiceConfigured(service.keyEnv)
        : true;
