...
const COMTRADE_NUMERIC_TO_ISO3: Record<string, string> = Object.entries(
  ISO3_TO_COMTRADE_NUMERIC,
).reduce(
  (acc, [iso3, num]) => {
    acc[num] = iso3;
    return acc;
  ...
  const { data, origin } = await withMockFallback(
    ...
    async () => {
      ...
      if (type === "summary") {
        ...
        const partners = (payload?.data || [])
          ...
          .map((d) => ({
            country:
              COMTRADE_NUMERIC_TO_ISO3[String(d.partnerCode)] ??
              d.partnerDesc ??
              "Unknown",
    ...
    () => {
      ...
      if (historicalRecords.length > 0) {
        if (type === "summary") {
          const record2024 = historicalRecords.find(
            (r) => r.year === 2024 && r.partnerCountry === "World",
          );
          const record2023 = historicalRecords.find(
            (r) => r.year === 2023 && r.partnerCountry === "World",
          );
          ...
          if (record2024 && record2023 && record2023.importValueUsd > 0) {
            yoy =
              (record2024.importValueUsd - record2023.importValueUsd) /
              record2023.importValueUsd;
          }
      ...
      if (type === "partners")
        return {
          partners: fallback.competitors.map((c) => ({
            country: c.country,
            value: (c.sharePct / 100) * fallback.importVolumeUsd,
          })),
      ...
      if (type === "trend") {
        ...
        if (witsTrend.length > 0) {
          ...
          return {
            trend: witsTrend.map((t) => ({
              year: t.year,
              value: Math.round(t.value * ratio),
            })),
            dataOrigin: "mock-fallback",
          };
        }
        return {
          trend: [
            { year: 2022, value: fallback.importVolumeUsd * 0.9 },
            { year: 2023, value: fallback.importVolumeUsd * 0.95 },
            { year: 2024, value: fallback.importVolumeUsd },
          ],
          dataOrigin: "mock-fallback",
        };
      }
