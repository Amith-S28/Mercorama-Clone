...
export async function POST(request: NextRequest) {
  if (!env.COMTRADE_API_KEY) {
    return NextResponse.json(
      { error: "COMTRADE_API_KEY is not configured in .env.local" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (!hsCode || hsCode.length < 4) {
      return NextResponse.json(
        { error: "Invalid or missing HS Code" },
        { status: 400 },
      );
    }
    ...
    for (const country of countries) {
      ...
      if (trendRes.ok) {
        ...
        for (const record of data) {
          newRecords.push(
            `${country},${hsCode},${record.period},World,${record.primaryValue}`,
          );
        }
      }
      ...
      if (partnersRes.ok) {
        ...
        for (const record of data) {
          ...
          const partnerIso =
            COMTRADE_NUMERIC_TO_ISO3[String(record.partnerCode)] ??
            record.partnerDesc ??
            "Unknown";
          newRecords.push(
            `${country},${hsCode},2024,${partnerIso},${record.primaryValue}`,
          );
        }
      }
    ...
    if (newRecords.length === 0) {
      return NextResponse.json(
        {
          error:
            "No data retrieved from UN Comtrade. Key might be rate-limited or invalid.",
        },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully downloaded and updated actual trade dataset for HS Code ${hsCode}`,
    });
  } catch (error: unknown) {
